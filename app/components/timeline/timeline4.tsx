...
const TimelineItem = ({ item, isActive, onClick }: { item: typeof timelineItems[0], isActive: boolean, onClick: () => void }) => {
  return (
    <div className="flex flex-col items-center snap-center w-40 cursor-pointer" onClick={onClick}>
      <div className={`w-4 h-4 rounded-full ${isActive ? 'bg-teal-500' : 'bg-gray-300'}`} />
      <div className="h-8 w-px bg-gray-300" />
      <div className="text-sm font-semibold text-gray-600">{item.year}</div>
    </div>
  )
}
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="relative mb-8">
          ...
          <div className="relative flex justify-between items-center">
            ...
            <div
              ref={containerRef}
              className="flex-1 overflow-x-scroll scrollbar-hide mx-4 snap-x snap-mandatory"
            >
              <div className="flex">
        ...
        >
          <h2 className="text-2xl font-bold text-teal-600 mb-2">{timelineItems[activeIndex].year}</h2>
          <h3 className="text-xl font-semibold text-gray-800 mb-4">{timelineItems[activeIndex].title}</h3>
          <p className="text-gray-600">{timelineItems[activeIndex].description}</p>
        </motion.div>

        {/* Code toggle button and display logic */}
      </div>
    </div>
  )
}
